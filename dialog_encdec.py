...
class DialogEncoderDecoder(Model):
    ...
    def build_encoder_function(self):
        if not hasattr(self, 'encoder_fn'):
            if self.bidirectional_utterance_encoder:
                ...
                self.encoder_fn = theano.function(inputs=[self.x_data, self.x_data_reversed, self.x_max_length], outputs=[h, hs], \
    ...
    def __init__(self, state):
        ...
        if self.bidirectional_utterance_encoder:
            ...
            self.params = [self.W_emb] + self.dcgm_encoder.params + self.utterance_decoder.params
            assert len(set(self.params)) == (len([self.W_emb]) + len(self.dcgm_encoder.params) + len(self.utterance_decoder.params))
        else:
